...
Unless the previous version of my esteemed colleague [Guillermo L車pez](https://bitbucket.org/guillermooo/virtualenvwrapper-powershell/overview) equivalent but obsolete it's compatible with Python 2+ and entierly based on a PowerShell script.
...
# Instalaci車n Mejorada y Soluci車n de Problemas (PowerShell Core / PS 7+) ??
## Curated by RS Montalvo

Esta documentaci車n complementa las instrucciones originales de `virtualenvwrapper-powershell`, proporcionando un m谷todo de instalaci車n manual robusto y soluciones para los *bugs* espec赤ficos encontrados en **PowerShell Core (PS 7+)** que impiden la carga correcta y el funcionamiento de `workon`.

Adem芍s, se incluye la configuraci車n de **Oh My Posh** para integrar el indicador del entorno virtual de Python (`(venv)`) directamente en el *prompt* personalizado.

## 1\. Instalaci車n Manual y Organizaci車n del M車dulo

La instalaci車n manual garantiza que el m車dulo se cargue en la ruta moderna de PowerShell Core y que puedas aplicar las correcciones necesarias al c車digo.

### A. Preparaci車n de Archivos

1.  **Rutas Clave:** Confirma que tu directorio de m車dulos sea la ruta moderna (sin "Windows"):

      * `C:\Users\TuUsuario\Documents\PowerShell\Modules`

2.  **Creaci車n de la Carpeta:** Crea la carpeta del m車dulo:

    ```powershell
    mkdir C:\Users\TuUsuario\Documents\PowerShell\Modules\VirtualEnvWrapper
    ```

3.  **Archivos del M車dulo:** Descarga `VirtualEnvWrapper.psm1` del repositorio y col車calo en la nueva carpeta. (Opcional, pero recomendado) Crea un archivo **`VirtualEnvWrapper.psd1`** (Manifiesto del M車dulo) para adherirte a las buenas pr芍cticas de PowerShell.
...
### B. Correcciones Cr赤ticas de C車digo (Bug de PS 7+)

Se deben realizar las siguientes correcciones directamente en el archivo `VirtualEnvWrapper.psm1` para evitar fallos de 芍mbito (*scope*) y la creaci車n incorrecta del directorio `~\Envs`.

1.  **Corregir la Activaci車n (`workon`):** El comando nativo de activaci車n debe usar *dot-sourcing* para modificar el *prompt* de PowerShell.

      * **Ubicaci車n:** Funci車n `Workon` (alrededor de la l赤nea 318).
      * **Cambiar:**
        ```powershell
      ...
      * **A:**
        ```powershell
        . $activate_path # Usa dot-sourcing para ejecutar en el 芍mbito actual
        ```

2.  **Corregir el Fallo de Inicializaci車n (Bug de Asignaci車n):** La l車gica de asignaci車n inicial falla al leer la variable de entorno, causando que el m車dulo siempre use el valor por defecto (`~\Envs`).

      * **Soluci車n Quir迆rgica:** Reemplazar el bloque de inicializaci車n para forzar la lectura correcta y eliminar la l車gica de contingencia fallida. (Ver el 迆ltimo c車digo propuesto).

-----

## 2\. Configuraci車n en el `$PROFILE`

El orden de las l赤neas en tu script de perfil (`$PROFILE`) es **CR赤TICO**. La variable `$env:WORKON_HOME` debe definirse *antes* de que el m車dulo sea importado.

Abre `C:\Users\TuUsuario\Documents\PowerShell\Microsoft.PowerShell_profile.ps1` y usa la siguiente estructura:

```powershell
# --- Configuraci車n de virtualenvwrapper-powershell ---

# 1. DEFINIR la ruta de los entornos virtuales (CR赤TICO: PRIMERO!)
...
# 3. IMPORTAR el m車dulo (SEGUNDO!)
Import-Module VirtualEnvWrapper

# --- Configuraci車n de Oh My Posh ---

# Si usas Oh My Posh, carga el motor y tu tema aqu赤:
oh-my-posh init pwsh --config 'C:\Users\TuUsuario\MiTema.omp.json' | Invoke-Expression
```

-----

## 3\. Integraci車n con Oh My Posh
...
### A. Verificar la Integraci車n

Aseg迆rate de que tu archivo de configuraci車n de Oh My Posh (ej: `MiTema.omp.json`) contenga un segmento con el siguiente tipo:
...
```
### En este ejemplo
El `$PROFILE` despu谷s de la la l赤nea `Set-PSReadLineOption -EditMode Emacs` que define el modo edici車n y atajos de teclado tipo `EMACS` la definic車n de tema `oh-my-posh` se lee:
```powershell
oh-my-posh init pwsh --config "$env:POSH_THEMES_PATH\quick-term.omp.json" | Invoke-Expression
```

### B. Soluci車n al Conflicto de Aliases

En PowerShell Core, el *bug* de alias *case-insensitive* puede afectar funciones clave como `workon`, por ello estos ajustes se han montado en una capa posterior como es el $PROFILE. En una versi車n siguiente y quiz芍 independiente de este ajuste, o `TO-DO` el ajuste ser赤a evitar la creaci車n del `alias` `workon` (o crearlo aociado a nombre matriz diferente e.g. `invoke-Workon`) pero en vista que PS es `case insensitive` tampoco har赤a falta. 
